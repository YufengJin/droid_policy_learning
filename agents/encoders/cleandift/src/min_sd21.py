...
class Attention(nn.Module):
    def __init__(self, inner_dim, cross_attention_dim=None, num_heads=None, dropout=0.0):
        ...
        self.to_out = nn.ModuleList([nn.Linear(inner_dim, inner_dim), nn.Dropout(dropout, inplace=False)])
    ...
    def forward(self, hidden_states, encoder_hidden_states=None):
        ...
        attn_output = F.scaled_dot_product_attention(q, k, v, dropout_p=0.0, scale=self.scale)
...
class Transformer2DModel(nn.Module):
    def __init__(self, in_channels, out_channels, n_layers):
        ...
        self.transformer_blocks = nn.ModuleList([BasicTransformerBlock(out_channels) for _ in range(n_layers)])
...
class CrossAttnDownBlock2D(nn.Module):
    def __init__(self, in_channels, out_channels, n_layers, has_downsamplers=True, conv_shortcut=True):
...
class UpBlock2D(nn.Module):
    def __init__(self, in_channels, out_channels, prev_output_channel, has_upsamplers=True):
...
class UNetMidBlock2DCrossAttn(nn.Module):
    def __init__(self, in_features):
        ...
        self.attentions = nn.ModuleList([Transformer2DModel(in_features, in_features, n_layers=1)])
        self.resnets = nn.ModuleList(
            [
    ...
    def __init__(self):
        ...
        self.up_blocks = nn.ModuleList(
            [
                ...
                CrossAttnUpBlock2D(
                    in_channels=320, out_channels=320, prev_output_channel=640, n_layers=1, has_upsamplers=False
                ),
            ]
        )
