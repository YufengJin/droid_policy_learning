"""
ALOHA DDP (Distributed Data Parallel) training entry point.

使用 ALOHADataset 读取 ALOHA HDF5 数据（非标准格式，每个文件一个 episode），支持 MP4 视频。
...
Usage:
  ...
  torchrun --nproc_per_node=8 -m robomimic.scripts.train_aloha \
      ...
      train.batch_size=32 experiment.name=aloha_diffusion

  # 单卡:
  python -m robomimic.scripts.train_aloha

Config 通过 Hydra 从 robomimic/scripts/train_configs/train_aloha.yaml 读取。
"""
...
from robomimic.utils.aloha_dataset import ALOHADataset
...
def get_config_from_args():
    ...
    with initialize_config_dir(config_dir=os.path.abspath(config_dir), version_base="1.1"):
        cfg = compose(config_name="train_aloha", overrides=overrides)
    ...
    if exp_name is None or exp_name == "null" or (isinstance(exp_name, str) and exp_name.strip() == ""):
        ...
        if isinstance(data_cfg, list) and len(data_cfg) > 0:
            ...
            dataset_str = os.path.basename(first_path.rstrip("/"))
            if len(data_cfg) > 1:
                dataset_str += "_plus{}".format(len(data_cfg) - 1)
        else:
            dataset_str = "aloha"
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        config.experiment.name = "{}_{}_{}_aloha".format(algo_name, dataset_str, timestamp)
...
def train_aloha(config, device, rank, world_size, local_rank, use_ddp, debug=False):
    ...
    # -----------------------------------------------------------------------
    # Load ALOHA dataset (directory scanning: one HDF5 per episode)
    # -----------------------------------------------------------------------
    ...
    if hasattr(config.observation, "image_dim") and config.observation.image_dim is not None:
        ...
        if isinstance(img_dim, (list, tuple)) and len(img_dim) >= 1:
            image_size = img_dim[0]  # square for ALOHA

    trainset = ALOHADataset(
        data_dir=data_dir,
    ...
    # ALOHA validation: try loading from 'val' subdirectory
    validset = None
    if config.experiment.validate:
        try:
            validset = ALOHADataset(
                data_dir=data_dir,
                obs_keys=all_obs_keys,
                action_keys=config.train.action_keys,
                seq_length=config.train.seq_length,
    ...
    env_meta = {"env_name": "ALOHA", "type": 1, "env_kwargs": {}}
...
def main():
    ...
    try:
        ...
        train_aloha(config, device=device, rank=rank, world_size=world_size, local_rank=local_rank, use_ddp=use_ddp, debug=debug)
